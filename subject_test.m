...
gammaStruct.display.forceLinearGamma    = true;
gammaStruct.display.gamma.power         = 1;

% trial conditions (will not overwrite existing file)
session         = NaturalStraightening.createTrialMatrix(subjectName, 'windowSize', 2, 'numRepetitions', 11);
...
while(~session.endExperiment)
    ...
    if(session.endExperiment)
        ...
        p = pldaps(@NaturalStraightening.setup, subjectName, gammaStruct);
