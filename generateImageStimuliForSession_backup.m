function p = generateImageStimuliForSession_backup(p)
...
outer_mask              = imresize(flattop8,[outer_diam_pixels, outer_diam_pixels]);

% Inner vignette
inner_mask              = ones(size(outer_mask));
padding_size            = floor((outer_diam_pixels-inner_diam_pixels)/2);
inner_x                = 1+padding_size : 1+padding_size+inner_diam_pixels-1;
inner_y                = 1+padding_size : 1+padding_size+inner_diam_pixels-1;
inner_mask(inner_x, inner_y) = imresize(1-flattop8,[inner_diam_pixels, inner_diam_pixels]);
...
for i = 1:length(texture_pointers)
    ...
    img                 = imresize(img, [outer_diam_pixels, outer_diam_pixels]);
    ...
    % No need to apply outer mask in this experiment
    %img                 = img .* outer_mask;
    img                 = img .* inner_mask;
