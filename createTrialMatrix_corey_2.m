function session = createTrialMatrix_corey_2(initials, varargin)
...
% Create pseudo-random array of movies 
%-------------------------------------------------------------------------%
% output: moviesAndSizes - cell array contains randomized movies at
% different sizes (array columns: 1=movie type, 2=movie num, 3=stimulus size)

% steps:
% 1) specify movies
% 2) assign movie sizes in a pseudorandom manner
% 3) pseudo-randomize natural and sythetic movie-size pairs
...
frames_movies = [naturalMovies, syntheticMovies];
frames_sizes = [natSizes, synSizes];
for mov=1:numMovies
...
clear startI
rand_index = randperm(size(blockParameters, 1));
blockParameters = blockParameters(rand_index,:);
...
for i = 1:size(blockParameters,1) % for each pre-defined block...
    block_specs = blockParameters(i,:);
    type_i      = block_specs(1);
    movie_i     = block_specs(2);
    size_i      = block_specs(3);
    
    frame_index = find((frames_movies == movie_i) & (frames_sizes == size_i));
    ...
    exptFrameInfo((i-1)*numTrialsPerBlock+1:i*numTrialsPerBlock,4:5) = frames{frame_index}(1:numTrialsPerBlock,1:2);
    % ...eliminate added frames from array of pre-randomized frames
    frames{frame_index}(1:numTrialsPerBlock,:) = [];
end
